...
# Import the new kernel (assuming you saved it in src/cortex)
try:
    from cortex.CortexKernel import CortexKernel
except ImportError:
    # Fallback class if file not made yet
    class CortexKernel:
        def __init__(self, r, l): pass
        def process_query(self, q, f, cb): cb("Error: CortexKernel not found.")
...
class AIChatPane(ttk.Frame):
    def __init__(self, parent, context_getter, library_root):
    ...
    def send_message(self):
        ...
        # 2. Init Kernel (Refresh data)
        self.kernel = CortexKernel(registry, self.library_root)
        
        # 3. Pass task to Kernel
        # We pass a lambda as a callback so the Kernel can talk back to the UI
        self.append_system("Cortex is thinking...")
        self.kernel.process_query(msg, selected_files, self.handle_response)

    def handle_response(self, text):
        # Must schedule UI update on main thread
        self.after(0, lambda: self.append_message("ai", text))
