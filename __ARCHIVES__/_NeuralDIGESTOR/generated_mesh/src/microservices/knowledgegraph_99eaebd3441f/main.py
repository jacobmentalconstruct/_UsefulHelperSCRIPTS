from fastapi import FastAPI, Request, HTTPException
        ...
        app = FastAPI(title="KnowledgeGraph API")
        ...
        async def execute(request: Request):
            ...
            try:
        if "CLASS" == "FUNCTION":
            result = logic.KnowledgeGraph(*args, **kwargs)
            return {"status": "success", "data": result}
        
        # Handle Class Instantiation
        if not method_name or method_name == "__init__":
            instance_store["instance"] = logic.KnowledgeGraph(*args, **kwargs)
            return {"status": "success", "message": "KnowledgeGraph initialized"}
