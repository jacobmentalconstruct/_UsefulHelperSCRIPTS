...
BAD_FROM_RE = re.compile(
    r"^(?P<indent>\s*)from\s+microservice_std_lib\s+import\s+(?P<items>.+?)\s*$",
    re.MULTILINE,
)
...
def fix_file(path: Path) -> bool:
    ...
    def repl(m: re.Match) -> str:
        ...
        changed = True
        return f"{indent}from microservice_std_lib import {join_import_items(items)}"
    ...
    if uses_baseservice and not has_base_import:
        ...
        if insert_after is not None:
            text = text[:insert_after] + "\nfrom base_service import BaseService" + text[insert_after:]
        else:
