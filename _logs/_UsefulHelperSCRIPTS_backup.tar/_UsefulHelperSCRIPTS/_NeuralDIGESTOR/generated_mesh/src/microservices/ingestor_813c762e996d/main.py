from fastapi import FastAPI, Request, HTTPException
        ...
        app = FastAPI(title="Ingestor API")
        ...
        async def execute(request: Request):
            ...
            try:
        if "CLASS" == "FUNCTION":
            result = logic.Ingestor(*args, **kwargs)
            return {"status": "success", "data": result}
        
        # Handle Class Instantiation
        if not method_name or method_name == "__init__":
            instance_store["instance"] = logic.Ingestor(*args, **kwargs)
            return {"status": "success", "message": "Ingestor initialized"}
