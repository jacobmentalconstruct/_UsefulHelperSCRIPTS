from fastapi import FastAPI, Request, HTTPException
        ...
        app = FastAPI(title="normalize_text API")
        ...
        if "FUNCTION" == "FUNCTION":
            result = logic.normalize_text(*args, **kwargs)
            return {"status": "success", "data": result}
        
        # Handle Class Instantiation
        if not method_name or method_name == "__init__":
            instance_store["instance"] = logic.normalize_text(*args, **kwargs)
            return {"status": "success", "message": "normalize_text initialized"}
