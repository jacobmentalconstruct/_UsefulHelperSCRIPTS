from fastapi import FastAPI, Request, HTTPException
        ...
        app = FastAPI(title="PythonParser API")
        ...
        async def execute(request: Request):
            ...
            try:
        if "CLASS" == "FUNCTION":
            result = logic.PythonParser(*args, **kwargs)
            return {"status": "success", "data": result}
        
        # Handle Class Instantiation
        if not method_name or method_name == "__init__":
            instance_store["instance"] = logic.PythonParser(*args, **kwargs)
            return {"status": "success", "message": "PythonParser initialized"}
